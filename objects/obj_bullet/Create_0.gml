speed = 5;
bounce = 0;       // amount of times it will bounce until is destroyed
can_wrap = false; // can wrap the map
damage = 1;       // damage that it deals to the asteroids
debris = 0;       // amount of debris generated upon destruction
life_time = 5 * room_speed; // life_span in ticks
rotation_speed = 0; // bullet speed rotation

last_collision = noone; // reference to the last object it collided with
last_collision_timer = 0;

parent = -1;